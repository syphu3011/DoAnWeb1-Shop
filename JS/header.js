...
function createListType(sex) {
  ...
  for (let i = 0; i < data.largeClassify.length; i++) {
    ...
    for (let j = 0; j < data.largeClassify[i].miniClassify.length; j++) {
      if (
        data.largeClassify[i].miniClassify[j].id.indexOf(sex.toUpperCase()) !=
          -1 ||
    ...
    if (showcart.style.display == "flex") {
      ...
      if (currentUser.cart.length > 0) {
        ...
        for (let i = 0; i < currentUser.cart.length; i++) {
          ...
          price.appendChild(
            document.createTextNode(calculated(arrProduct[i].price) + "VND")
          );
          ...
          btnSub.onclick = function () {
            if (count > 1) {
              ...
              let t =
                (parseInt(currentUser.cart[i].price) /
                  parseInt(currentUser.cart[i].amount)) *
                count;
          ...
          text.appendChild(
            document.createTextNode(inStock[i].amount + " sản phẩm có sẵn")
          );
...
function search(text) {
  ...
  for (let i = 0; i < data.product.length; i++) {
    if (
      data.product[i].id.toLowerCase().indexOf(text.toLowerCase()) != -1 ||
