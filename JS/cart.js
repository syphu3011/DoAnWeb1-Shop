...
function createCart() {
    if (currentUser.cart.length > 0) {
        ...
        for (let i = 0; i < currentUser.cart.length; i++) {
            ...
            if (sizeP[i] != null) {
                sizeP[i].forEach(element => {
                    if (element.id == currentUser.cart[i].idSize) {
                        str += ` <div class="size" style="border-color: red;">
                   ` + element.name + `
                            </div>`
                    } else {
                        str += ` <div class="size">
            ...
            let pr = 0
            prInStock.forEach(e => {
                if (currentUser.cart[i].idProd == e.idProd &&
                    ...
                    <td>
                        <div>
                            ...
                            <div style="color: gray;
                                ...
                                line-height: 1.5;
                                font-size: 10px">
                                có  <span>` + pr + ` </span>sản phẩm có sẵn
                            </div>
                        </div>
                    </td>
                    <td>
                    <div class="chon-size">` + str + `</div>
                    </td>
                    <td class="ton-tien">
...
function tinhtongtien(count, id, cost) {
    for (let i=0;i<KM.length;i++){
        if (KM[i].idProd == id) {
            return (parseInt(cost) - parseInt(cost) * parseInt(KM[i].discount_percent) / 100 - parseInt(KM[i].discount_price)) * count
        }
    }
...
function checkConHang(list) {
    for (let i = 0; i < list.length; i++) {
        for (let j = 0; j < prInStock.length; j++) {
            if (list[i].idProd == prInStock[j].idProd &&
...
function sukien() {
    ...
    for (let i = 0; i < btnXoa.length; i++) {
        btnXoa[i].onclick = function() {
            while (document.getElementsByClassName("table-giohang")[0].rows.length > 0) {
            ...
            setTimeout(() => {
                    ...
                    tongtien = 0
                    document.getElementById("tongthanhtoan").textContent = calculated(tongtien) + " VND"
                    createCart()
                }, 700)
    ...
    for (let i = 0; i < proSize.length; i++) {
        if (proSize[i].textContent.length != 0) {
            for (let j = 0; j < proSize[i].getElementsByClassName("size").length; j++) {
                // console.log(i, j)

                proSize[i].getElementsByClassName("size")[j].onclick = function() {
                    let ind = false
                    prInStock.forEach(el => {
                        if (el.idSize == sizeP[i][j].id &&
                    ...
                    if (ind) {
                        ...
                        document.getElementById("hienthigiohang").style.display = ""
                        document.getElementById("noti").style.display = "flex"
                        document.getElementById("noti-noti").innerHTML = "cập nhật giỏ hàng Thành công"
                        showacc(document.getElementById("noti-noti"), -500, 0)
                        document.getElementById("noti-noti").style.display = "flex"
                        setTimeout(() => {
                            document.getElementById("noti").style.display = ""
                ...
                if (currentUser.cart[i].amount > 1) {
                    currentUser.cart[i].amount--
                        currentUser.cart[i].price = tinhtongtien(
                            ...
                            pro[i].price)
                    document.getElementsByClassName("hien-sl")[i].textContent = currentUser.cart[i].amount
                    if (btnCheckBox[i].checked) {
                        tongtien -= pro[i].price
                    }
                    document.getElementsByClassName("ton-tien")[i].textContent = calculated(currentUser.cart[i].price) + " VND"
                    document.getElementById("tongthanhtoan").textContent = calculated(tongtien) + " VND"
    ...
    for (let i = 0; i < btnTang.length; i++) {
        btnTang[i].onclick = function() {
            if (currentUser.cart[i].amount != 0) {
                if (parseInt(currentUser.cart[i].amount) < parseInt(prInStock[i].amount)) {
                    currentUser.cart[i].amount++
                        currentUser.cart[i].price = tinhtongtien(
                            currentUser.cart[i].amount,
                            currentUser.cart[i].idProd,
