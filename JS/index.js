...
function checkDate(birthday) {
    if (parseInt(getCurrentDate().split(" ")[0].split("/")[2]) - parseInt(birthday.split("-")[0]) < parseInt(data.variable[0].gh_duoi) ||
        parseInt(getCurrentDate().split(" ")[0].split("/")[2]) - parseInt(birthday.split("-")[0]) > parseInt(data.variable[0].gh_tren)) {
        return false
    }
    return true
}
...
btnsigup.onclick = function() {
        ...
        for (let i = 0; i < 3; i++) {
            if (s[i].checked) {
                ...
                sex = document.getElementById("update-in4").getElementsByTagName("label")[i].textContent
            }
        }
        if (checkValid(
                firstName.value,
                ...
                birthday.value,
                sex)) {
            if (checkValidNameU(firstName.value,
                    lastName.value)) {
                    ...
                    if (checkSamePassword(password_regis.value, same_passwd.value)) {
                        if (checkDate(birthday.value)) {
                            let customer = new Customer(
                                ...
                                password_regis.value,
                                sex,
                                birthday.value.split("-")[2] + "/" + birthday.value.split("-")[1] + "/" + birthday.value.split("-")[0])
    ...
    //đăng nhập
document.getElementById("mailorphone").addEventListener("keypress", function(ev) {
    if (ev.key === "Enter") {
        login()
    }
})
        ...
        if (!lock) {
            if (!isStaff) {
                ...
                setTimeout(() => {
                    ...
                    account.style.display = ""
                    document.getElementById("noti").style.display = "flex"
                    document.getElementById("noti-noti").innerHTML = "Đăng nhập thành công"
                    showacc(document.getElementById("noti-noti"), -500, 0)
                    document.getElementById("noti-noti").style.display = "flex"
                    setTimeout(() => {
                        document.getElementById("noti").style.display = ""
                ...
                document.getElementById("update-birthday").value = currentUser.birth_day
                ...
                let se = document.getElementById("update-in4").getElementsByTagName("label")
                for (let i = 0; i < 3; i++) {
        ...
        if (id != "") {
            ...
            for (let k = 0; k < t.length; k++) {
                document.getElementById(id).getElementsByClassName("list-item")[k].onclick = function() {
                    console.log(document.getElementById(id).getElementsByClassName("list-item")[k].textContent)
...
function getPromote(id) {
    for (let i = 0; i < data.promote.length; i++) {
        for (let j = 0; j < data.promote[i].products.length; j++) {
            if (data.promote[i].products[j].id.toLowerCase()==id.toLowerCase() ) {
                return [i, j]
            }
        }
    }
...
function createListProduct(path, img, nameProduct, price, sale, madeIn, id, clasify, description) {
    ...
    liItem.onclick = function() {
        if (!isCTSP) {
            isCTSP = true
            document.getElementById("div-onClickProduct").style.display = "flex"
            xemCTSP(id)
...
saveUpdate.onclick = function() {
    ...
    for (let i = 3; i < s.length; i++) {
        if (s[i].checked) {
            // console.log(s[i].value)
            sex = document.getElementById("update-in4").getElementsByTagName("label")[i - 3].textContent
        }
    }
    ...
    if (!correct) { return }
    if (update_name.value != "" && update_contact.value != "" && update_birthday.value != "" && sex != "") {
        if (checkValidNameU(update_name.value, "")) {
            if (checkValidPhoneNumber(update_contact.value)) {
                if (checkDate(update_birthday.value)) {
                    if (tdmk) {
                        if (checkUpdatePassword(document.getElementById("mkht").value) &&
...
function createReceipt(liProd) {
    let st = "Chờ xác nhận"
    let newRC = new Receipt(initIdReceipt(), currentUser.id, liProd, currentUser.name, getCurrentDate(), document.getElementById("inp-dc").value, st)
    return newRC
}
