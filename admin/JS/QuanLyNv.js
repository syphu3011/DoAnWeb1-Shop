...
`
        ...
        function captaikhoan() {
            ...
            //     if (staff.phone.trim() == number_phone) {
        ...
        function renderTable1() {
            ...
            for (let i = 0; i < arrNhanVien.length; i++) {
                if (arrNhanVien[i].id_status.toLowerCase().includes("hết làm việc")) {

                } else {
                    if (arrNhanVien[i].id == "") arrNhanVien[i].id = ("NV000" + (i + 1)).toLowerCase();
                    if (arrNhanVien[i].id_status == "") arrNhanVien[i].id_status = "1";
        ...
        function oke() {
            ...
            for (let i = 0; i < arrNhanVien.length; i++) {
                if (arrNhanVien[i].id.toLowerCase() == selectedNv.toLowerCase()) {
                    arrNhanVien[i].id_status = "hết làm việc";
                    // objk.staff = arrNhanVien
                    // writeToLocalStorage(objk)
                }
        ...
        function SearchNv() {
            ...
            if (timtheo == "tatca") {
                newArrNV = arrNhanVien.filter((item) =>
                    ...
                    item.id_status.trim().toLowerCase().includes(trangthai) &&
                    item.birthday.toLowerCase().includes(toDDMMYYYY(ngsinh)) &&
            ...
            if (timtheo == "id") {
                newArrNV = arrNhanVien.filter((item) =>
                    item.id.toLowerCase().includes(aidi) &&
                    item.id_status.toLowerCase().includes(trangthai) &&
                    item.birthday.toLowerCase().includes(toDDMMYYYY(ngsinh)) &&
                    item.date_init.toLowerCase().includes(toDDMMYYYY(ngaythamgia))
                )
                for (let i = table.rows.length - 1; i > 0; i--)
